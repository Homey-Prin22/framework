...
rooms = ['area1_room1', 'area1_room2', 'area2_room1', 'area2_room2', 'area1_lab1', 'area2_server_room', 'area1_storage']
...
def basevalue_pollution(room):
    if room == 'area2_server_room':
        base_value = random.uniform(100, 200) 
    elif room == 'area1_lab1':
...
def generate_environment_data(sensor_id):
    while running:
        ...
        for room in rooms:
            ...
            # Split room name into area and room
            area, room_name = room.split('_', 1)
            mqtt_topic = f"{area}/{room_name}"
            sensor_name = f"{sensor_id}{room}"
