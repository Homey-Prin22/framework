...
def sensor_list():
    with open("/framework/code/querying/output.json", "r") as file:
        data = json.load(file)
...
def allowed_param_list(username, allowed_sensor_list):
    with open("/framework/code/querying/output.json", "r") as file:
        data = json.load(file)
...
def build_query(username, allowed_sensor_list, source_id, param, aggr="", limit="", offset="", start="", stop=""):
    ...
    if param:
        sensor_list2 = []
        for key, value in data.items():
            if (param in value) and (key in allowed_sensor_list):
...
def get_data():
    ...
    allowed_sensor_list_raw = request.headers.get("X-Allowed-Sensors")

    if allowed_sensor_list_raw:
        try:
            allowed_sensor_list = json.loads(allowed_sensor_list_raw)
        except json.JSONDecodeError:
            return Response(json.dumps({"error": "Invalid allowed sensor list format"}), content_type='application/json', status=400)
    else:
        allowed_sensor_list = None
    ...
    if source_id and source_id not in allowed_sensor_list:
        return Response(json.dumps({"error": f"Invalid 'source_id' value, must be one of {allowed_sensor_list}"}), content_type='application/json', status=400)
    
    if param:
        valid_params = allowed_param_list(username, allowed_sensor_list)
        if not valid_params:
            return Response(json.dumps({"error": f"You are not allowed to perform parameter query"}), content_type='application/json', status=400)
    ...
    try:
        final_query = build_query(username, allowed_sensor_list, source_id, param, aggr, limit, offset, start_time, stop_time)
        if final_query is None:
            return Response({"error: Query not allowed"}, content_type='application/json', status=400)
